...
// server/routes/telegram.js - the client only ever sends an idea id (plus a
// 'ru'/'en' language choice), never the post content itself.
...
router.post('/vk', async (req, res) => {
    ...
    const result = await publishToVk(idea, { accessToken: settings.access_token, groupId: settings.group_id }, coverAsset?.url || null, lang);
...
router.post('/instagram', async (req, res) => {
    ...
    const coverAsset = await loadCoverAsset(idea);
    const result = await publishToInstagram(idea, { accessToken: settings.access_token, businessAccountId: settings.business_account_id }, coverAsset, lang);
